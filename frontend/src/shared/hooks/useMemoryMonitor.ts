...
import { captureMemorySnapshot } from '@/shared/lib/memory/snapshot';
import type {
  MemorySnapshot,
  MemoryStats,
...
export function useMemoryMonitor(
  ...
  const stats = useMemo((): MemoryStats => {
    ...
    // Current memory
    const current = toMB(snapshots[snapshots.length - 1].usedJSHeapSize ?? 0);

    // Peak memory
    const peak = Math.max(
      ...snapshots.map((s) => toMB(s.usedJSHeapSize ?? 0))
    );

    // Average memory
    const sum = snapshots.reduce(
      (acc, s) => acc + toMB(s.usedJSHeapSize ?? 0),
      0
    );
    ...
    if (snapshots.length >= 2) {
      ...
      const memoryDiffMB = toMB((last.usedJSHeapSize ?? 0) - (first.usedJSHeapSize ?? 0));
