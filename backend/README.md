...
# Deployment (Docker on own servers)
DEPLOY_DOMAIN=viably.dev
DEPLOY_SSL_ENABLED=false
...
└──────────────┘     └──────┬───────────┘
               ...
               └───────────┘    └──────┬──────┘
                         ...
                         │                            │
                   ┌─────▼──────┐            ┌───────▼────────┐
                   │ User Bots  │            │ User Websites  │
                   │ (Docker    │            │ (Nginx +       │
                   │  containers)│            │  Certbot SSL)  │
                   └────────────┘            └────────────────┘
```

- **Frontend**: Next.js (Docker, viably.dev)
- **Backend API**: FastAPI (Docker, api.viably.dev)
- **Worker**: Celery (Docker, background AI generation)
- **Database**: PostgreSQL (Docker, self-hosted)
- **Cache/Queue**: Redis (Docker, self-hosted)
- **User Bots**: Isolated Docker containers on viably-network (ports 10000-11000)
- **User Websites**: Nginx subdomains + Let's Encrypt SSL
...
- **Container Logs**: Dozzle (port 9999)
