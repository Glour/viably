...
async def test_trigger_generation_success(
    ...
    """Test triggering generation for draft project dispatches Celery task."""
    # Mock the Celery task to avoid actual generation
    ...
    assert response.status_code == 200
    data = response.json()
    # Endpoint now returns immediately with status "generating"
...
@pytest.mark.asyncio
async def test_trigger_generation_non_allowed_status(
    client: AsyncClient,
    auth_token: str,
    db_session: AsyncSession,
    test_project: Project,
) -> None:
    """Test triggering generation on project with non-allowed status returns 400."""
    # Set project to 'generating' status (not allowed for new generation)
    test_project.status = ProjectStatus.GENERATING.value
    ...
    assert "draft" in detail or "error" in detail or "ready" in detail
...
async def test_status_transitions(
    ...
    with patch("app.projects.service.process_generation") as mock_task:
        ...
        # Trigger generation — endpoint returns immediately with "generating"
        response = await client.post(
            f"/api/projects/{test_project.id}/generate",
            headers={"Authorization": f"Bearer {auth_token}"},
        )
    ...
    # Re-generation from "generating" status should fail (400)
    response = await client.post(
        f"/api/projects/{test_project.id}/generate",
        headers={"Authorization": f"Bearer {auth_token}"},
    )
