...
from api.src.ai.prompts import build_generation_prompt, extract_code_files
...
class TestGenerationService:
    ...
    async def test_generation_success(
        ...
        """
        from api.src.ai.service import AIGenerationService
...
"""
    ...
    async def test_generation_invalid_status(
        ...
        """Test that validation fails for projects not in draft, error, or ready status."""
        from api.src.ai.service import AIGenerationService
        ...
        project = Project(
            ...
            template_id=test_template.id,
            config={"bot_name": "GeneratingBot"},
            status=ProjectStatus.GENERATING.value,  # Not draft, error, or ready
            is_public=False,
            created_at=datetime.now(timezone.utc),
            updated_at=datetime.now(timezone.utc),
        )
        ...
        with pytest.raises(ValueError, match="Project status must be 'draft', 'error', or 'ready'"):
            await service.validate_project_for_generation(project.id)
...
class TestViewGeneratedCode:
    ...
    async def test_view_generated_code_after_generation(
        ...
        assert data["ai_model_used"] == "claude-sonnet-4-6"
...
class TestErrorHandlingWithCreditRefund:
    ...
    @pytest.mark.asyncio
    async def test_credits_deducted_on_generation_dispatch(
        self,
        client,
        auth_token: str,
        db_session: AsyncSession,
        ...
        """Test that credits are deducted when generation is dispatched (T017).

        With async Celery generation, the endpoint deducts credits and returns
        immediately. Credit refunds happen in the Celery task on failure.

        Verifies:
        1. Create project in draft status
        2. User has sufficient credits
        3. Trigger generation (Celery task mocked)
        4. Verify credits are deducted
        5. Verify response returns status "generating"
        """
        ...
        # Mock Celery task to avoid actual generation
        with patch("app.projects.service.process_generation") as mock_task:
        ...
        # Verify success (async dispatch)
        assert response.status_code == 200
        assert response.json()["status"] == "generating"

        # Refresh user to check credits were deducted
        await db_session.refresh(test_user)
        assert test_user.credits == initial_credits - test_template.credit_cost
    ...
    async def test_regeneration_from_error_status(
        ...
        with patch("app.projects.service.process_generation") as mock_task:
            ...
            # Trigger regeneration from error status
            response = await client.post(
                f"/api/projects/{project.id}/generate",
                headers={"Authorization": f"Bearer {auth_token}"},
            )
        ...
        assert response.status_code == 200
        data = response.json()
        # Endpoint returns immediately with "generating", error_message cleared
...
class TestCeleryWorkerConfiguration:
    ...
    def test_celery_app_configuration(self) -> None:
        ...
        from api.src.ai.worker import celery_app
    ...
    def test_exception_classification(self) -> None:
        ...
        from api.src.ai.worker import PERMANENT_EXCEPTIONS, RETRYABLE_EXCEPTIONS
    ...
    def test_process_generation_task_exists(self) -> None:
        ...
        from api.src.ai.worker import process_generation
