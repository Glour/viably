...
from api.src.emails.template_fallback import render_fallback_template
...
class EmailTemplateRenderer:
    ...
    async def render_template(
        ...
        """
        # If frontend emails directory doesn't exist, use fallback immediately
        if not self.emails_dir.exists():
            logger.info(
                "Frontend emails directory not found, using fallback template",
            ...
            try:
                ...
                raise RuntimeError(
                    f"Template '{template_name}' not found in fallback templates"
                )
        ...
        if not template_file.exists():
            logger.warning(
                "React Email template not found, using fallback",
                extra={"template": template_name},
            )
            try:
                return render_fallback_template(template_name, props)
